...
-(void)initTagsView
{
    ...
    for (int i = 0; i < self.tags.count; i ++) {
        UIButton *tagView = [self tagButtonWithText:self.tags[i]];
        ...
        lastTagButton = tagView;
        [self.tagButtons addObject:tagView];
    }
...
{
    UIView *lastTagButton = [self getLastTagButton];
    [self.enterTextfield mas_updateConstraints:^(MASConstraintMaker *make) {
        if (lastTagButton)
        {
            make.left.mas_equalTo(lastTagButton.mas_right).offset(5);
            make.top.mas_equalTo(lastTagButton.mas_top).offset(0);
